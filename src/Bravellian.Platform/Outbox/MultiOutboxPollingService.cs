...
/// </summary>
public sealed class MultiOutboxPollingService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (processedCount > 0)
                {
                    this.logger.LogDebug("Multi-outbox polling iteration completed: {ProcessedCount} messages processed", processedCount);
                }
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            {
                this.logger.LogDebug("Multi-outbox polling service stopped due to cancellation");
                break;
            }
            catch (Exception ex)
            {
                // Log and continue - don't let processing errors stop the service
                this.logger.LogError(ex, "Error in multi-outbox polling iteration - continuing with next iteration");
            }
        ...
        this.logger.LogInformation("Multi-outbox polling service stopped");
    }
}
