    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                if (deletedCount > 0)
                {
                    this.logger.LogInformation("Outbox cleanup completed: {DeletedCount} old messages deleted", deletedCount);
                }
                else
                {
                    this.logger.LogDebug("Outbox cleanup completed: no old messages to delete");
                }
            }
            catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
            {
                this.logger.LogDebug("Outbox cleanup service stopped due to cancellation");
                break;
            }
            catch (Exception ex)
            {
                // Log and continue - don't let cleanup errors stop the service
                this.logger.LogError(ex, "Error during outbox cleanup - continuing with next iteration");
            }
        ...
        this.logger.LogInformation("Outbox cleanup service stopped");
    }

    private async Task<int> CleanupOldMessagesAsync(CancellationToken cancellationToken)
    {
        this.logger.LogDebug("Starting outbox cleanup for messages older than {RetentionPeriod}", this.retentionPeriod);
        ...
        {
            this.logger.LogError(ex, "Failed to cleanup old outbox messages");
            throw;
        }
    }
}
