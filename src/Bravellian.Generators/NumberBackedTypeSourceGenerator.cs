    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(candidateFiles, static (productionContext, input) =>
        {
            try
            {
                ...
                {
                    GeneratorDiagnostics.ReportSkipped(productionContext, $"No output generated for '{input.Path}'. Ensure required <NumberEnum> elements or JSON fields are present.");
                    return;
                }
            ...
            {
                GeneratorDiagnostics.ReportError(productionContext, $"NumberBackedEnumTypeSourceGenerator failed for '{input.Path}'", ex);
            }
        });
    }
