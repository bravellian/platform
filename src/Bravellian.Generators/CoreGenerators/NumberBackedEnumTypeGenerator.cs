...
public static class NumberBackedEnumTypeGenerator
{
    public static GeneratorParams? GetParams(XElement xml, IBgLogger? logger, string sourceFilePath)
    {
        ...
        return new(attributes.TryGetValue("name"), attributes.TryGetValue("namespace"), true, numberType, enumValues, additionalProperties, sourceFilePath);
    }
    ...
    public static string? GenerateValueConverter(GeneratorParams? structToGenerate, IBgLogger? logger)
    {
        if (structToGenerate is { } sg)
        {
            return ValueConverterGenerator.GenerateNumberBackedEnumConverter(
                sg.Name,
                sg.Namespace,
                sg.NumberType);
        }
    ...
    private static string GenerateClass(GeneratorParams relatedClass)
    {
        ...
        var licenseHeader = GeneratorConfig.GetLicenseHeader();
        var partialKeyword = GeneratorConfig.GetPartialKeyword();

        var bravellianInterfaces = GeneratorConfig.IncludeBravellianInterfaces
            ? new[] { "Bravellian.IHasValueConverter" }
            : Array.Empty<string>();

        var interfaceList = bravellianInterfaces.Length > 0 
            ? " : " + string.Join(", ", bravellianInterfaces)
            : string.Empty;
...
public readonly {{partialKeyword}}record struct {{relatedClass.Name}}{{interfaceList}}
{
{{constValues}}
