    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(candidateFiles, static (productionContext, input) =>
        {
            try
            {
                ...
                {
                    GeneratorDiagnostics.ReportSkipped(productionContext, $"No output generated for '{input.Path}'. Ensure required <GuidBacked> elements or JSON fields are present.");
                    return;
                }
            ...
            {
                GeneratorDiagnostics.ReportError(productionContext, $"GuidBackedTypeSourceGenerator failed for '{input.Path}'", ex);
            }
        });
    }
