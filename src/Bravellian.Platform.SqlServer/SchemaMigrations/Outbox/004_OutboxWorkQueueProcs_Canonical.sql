...
BEGIN
    ...
    DELETE FROM [$SchemaName$].[$OutboxTable$]
     WHERE Status = 2
       ...
       AND COALESCE(ProcessedOn, ProcessedAt) < @cutoffTime;
...
BEGIN
    SET NOCOUNT ON;
    UPDATE o SET Status = 2, OwnerToken = NULL, LockedUntil = NULL,
        IsProcessed = 1,
...
BEGIN
    ...
    UPDATE o SET
        ...
        DueTimeUtc = COALESCE(@DueTimeUtc, o.DueTimeUtc, o.DueOn, @now),
        DueOn = COALESCE(@DueTimeUtc, o.DueOn, o.DueTimeUtc)
    FROM [$SchemaName$].[$OutboxTable$] o JOIN @Ids i ON i.Id = o.Id
    WHERE o.OwnerToken = @OwnerToken AND o.Status = 1;
END
GO
...
BEGIN
    ...
    UPDATE [$SchemaName$].[$OutboxTable$]
        SET Status = 0, OwnerToken = NULL, LockedUntil = NULL
